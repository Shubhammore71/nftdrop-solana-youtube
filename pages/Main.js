...
export default function Main() {
  ...
  const mint = async () => {
    ...
    setTxLoading(true);
    setTxError(null); // Corrected
  
    try {
      const mintResu = await metaplex.candyMachines().mint({
        candyMachine: {
          address: candyState.address,
          collectionMintAddress: candyState.collectionMintAddress,
          candyGuardAddress: candyState.candyGuardAddress,
        },
        collectionUpdateAuthority: candyState.authorityAddress,
        gorup: null // Typo: should be 'group'
      });
  
      console.log({ mintResu });
    } catch (e) {
      console.log(e);
      toast.error("Mint failed");
      setTxError(e.message);
    } finally {
      setTxLoading(false); // Corrected
